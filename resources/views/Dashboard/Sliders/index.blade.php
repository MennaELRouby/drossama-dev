<x-dashboard.layout :title="__('dashboard.sliders')">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">{{ __('dashboard.sliders') }}</h4>

                    <div class="page-title-right d-flex align-items-center">
                        <div class="btn btn-list">
                            <a href="{{ route('dashboard.sliders.create') }}" class="btn ripple btn-primary">
                        ...
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">{{ __('dashboard.sliders') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($sliders as $slider)
                                    <tr id="{{ $slider->id }}">
                                        ...
                                        <td><a
                                                href="{{ route('dashboard.sliders.edit', $slider->id) }}">{{ $slider->id }}</a>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.sliders.edit', $slider->id) }}">{{ $slider->getTranslation('title', 'en') }}</a>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.sliders.edit', $slider->id) }}">{{ $slider->getTranslation('title', 'ar') }}</a>
                                        </td>
                                        <td>
                                            <a href="{{ route('dashboard.sliders.edit', $slider->id) }}">
                                                <img src="{{ $slider->image_path }}" width="70" height="70"
                                        ...
                                        <td><a
                                                href="{{ route('dashboard.sliders.edit', $slider->id) }}">{{ $slider->type }}</a>
                                        </td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <form
                                                    action="{{ route('dashboard.sliders.toggle-status', $slider->id) }}"
                                                    method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="btn-action {{ $slider->status ? 'unpublish' : 'publish' }}"
                                                        title="{{ $slider->status ? __('dashboard.unpublish') : __('dashboard.publish') }}">
                                                        <i
                                                            class="fas fa-{{ $slider->status ? 'eye-slash' : 'eye' }}"></i>
                                                    </button>
                                                </form>
                                                <a href="{{ route('dashboard.sliders.edit', $slider->id) }}"
    ...
    @push('scripts')
        ...
        <script>
            ...
            $('#btn_active').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('dashboard.change.status', ['modelname' => 'sliders', 'ids' => ':ids']) }}"
            ...
            // Individual slider delete function

                    ...
                    <form action="{{ route('dashboard.sliders.update', [$slider->id]) }}" method="POST"
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-2">
                                ...
                                <input class="form-control" name="order" type="number" value="{{ $slider->order }}"
                            ...
                            <div class="form-group col-md-4 mb-3">
                                ...
                                <select name="language" class="form-control">
                                    @foreach (App\Models\Slider::getLanguageSelect() as $key => $label)
                                        <option value="{{ $key }}" @selected(old('language', $slider->language ?? 'all') == $key)>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-6">
                                <label for="">{{ __('dashboard.desktop_image') }}</label>
                                <img src="{{ $slider->image_path }}" width="250" class="img-thumbnail">
                            </div>

                            <div class="form-group col-md-6">
                                <label for="">{{ __('dashboard.mobile_image') }}</label>
                                @if ($slider->mobile_image)
                                    <img src="{{ $slider->mobile_image_path }}" width="250" class="img-thumbnail">
                                @else
                                    <div class="text-muted">{{ __('dashboard.no_mobile_image') }}</div>
                                @endif
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input class="form-control" name="alt_image" type="text"
                                    placeholder="{{ __('dashboard.alt_image') }}" value="{{ $slider->alt_image }}">
                            </div>

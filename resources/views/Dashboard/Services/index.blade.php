        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">{{ __('dashboard.services') }}</h4>

                    <div class="page-title-right d-flex align-items-center">
                        <div class="btn btn-list">
                            <a href="{{ route('dashboard.services.create') }}" class="btn ripple btn-primary">
                        ...
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">{{ __('dashboard.services') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($services as $service)
                                    <tr id="{{ $service->id }}">
                                        ...
                                        <td><a
                                                href="{{ route('dashboard.services.edit', $service->id) }}">{{ $service->getTranslation('name', 'en') }}</a>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.services.edit', $service->id) }}">{{ $service->getTranslation('name', 'ar') }}</a>
                                        </td>
                                        ...
                                        <td>
                                            <a href="{{ route('dashboard.services.edit', $service->id) }}">
                                                {{ $service->parent ? $service->parent->getTranslation('name', 'ar') : __('dashboard.no_parent') }}
                                            </a>
                                        </td>
                                        <td>
                                            @if ($service->status == 1)
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <form
                                                    action="{{ route('dashboard.services.toggle-status', $service->id) }}"
                                                    method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="btn-action {{ $service->status ? 'unpublish' : 'publish' }}"
                                                        title="{{ $service->status ? __('dashboard.unpublish') : __('dashboard.publish') }}">
                                                        <i
                                                            class="fas fa-{{ $service->status ? 'eye-slash' : 'eye' }}"></i>
                                                    </button>
                                                </form>
                                                <a href="{{ route('dashboard.services.edit', $service->id) }}"

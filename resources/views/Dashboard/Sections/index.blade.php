        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">{{ __('dashboard.sections') }}</h4>

                    <div class="page-title-right d-flex align-items-center">
                        <div class="btn btn-list">
                            <a href="{{ route('dashboard.sections.create') }}" class="btn ripple btn-primary">
                        ...
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">{{ __('dashboard.sections') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($sections as $section)
                                    <tr id="{{ $section->id }}">
                                        <td class="text-center">
                                            <div class="checkbox-enhanced">
                                                <input type="checkbox" name="checkbox"
                                                    class="check-inputs form-check-input" value="{{ $section->id }}">
                                                <span class="checkmark"></span>
                                            </div>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.sections.edit', $section->id) }}">{{ $section->id }}</a>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.sections.edit', $section->id) }}">{{ $section->getTranslation('name', 'en') }}</a>
                                        </td>
                                        <td><a
                                                href="{{ route('dashboard.sections.edit', $section->id) }}">{{ $section->getTranslation('name', 'ar') }}</a>
                                        </td>

                                        <td><a
                                                href="{{ route('dashboard.sections.edit', $section->id) }}">{{ $section->order }}</a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('dashboard.sections.edit', $section->id) }}">
                                                {{ $section->parent ? $section->parent->getTranslation('name', 'ar') : __('dashboard.no_parent') }}
                                            </a>
                                        </td>
                                        <td>
                                            @if ($section->status == 1)
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <form
                                                    action="{{ route('dashboard.sections.toggle-status', $section->id) }}"
                                                    method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="btn-action {{ $section->status ? 'unpublish' : 'publish' }}"
                                                        title="{{ $section->status ? __('dashboard.unpublish') : __('dashboard.publish') }}">
                                                        <i
                                                            class="fas fa-{{ $section->status ? 'eye-slash' : 'eye' }}"></i>
                                                    </button>
                                                </form>
                                                <a href="{{ route('dashboard.sections.edit', $section->id) }}"
            ...
            $('#btn_active').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('dashboard.change.status', ['modelname' => 'sections', 'ids' => ':ids']) }}"

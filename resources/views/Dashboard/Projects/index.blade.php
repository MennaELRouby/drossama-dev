                            ...
                            <tbody>
                                @foreach ($projects as $project)
                                    <tr id="{{ $project->id }}">
                                        ...
                                        <td>
                                            <a href="{{ route('dashboard.projects.edit', $project->id) }}">
                                                {{ $project->parent ? $project->parent->getTranslation('name', 'ar') : __('dashboard.no_parent') }}
                                            </a>
                                        </td>
                                        <td>
                                            @if ($project->status == 1)
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <form
                                                    action="{{ route('dashboard.projects.toggle-status', $project->id) }}"
                                                    method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="btn-action {{ $project->status ? 'unpublish' : 'publish' }}"
                                                        title="{{ $project->status ? __('dashboard.unpublish') : __('dashboard.publish') }}">
                                                        <i
                                                            class="fas fa-{{ $project->status ? 'eye-slash' : 'eye' }}"></i>
                                                    </button>
                                                </form>
                                                <a href="{{ route('dashboard.projects.edit', $project->id) }}"
